...
def market_price(product_id: UUID, as_of: Optional[date] = None,
                 tenant=Depends(get_tenant), session: Session = Depends(get_session)):
    """Champion P10/P50/P90 market price `horizon` days ahead (base currency, per product unit).
    Advisory only: it shows where the ceiling may move and never changes the envelope."""
    d = as_of or date.today()
    try:
        product = product_repo.get_product(session, tenant["tenant_id"], product_id)
    except LookupError as e:
        raise HTTPException(404, str(e))
    fc, skip = champion_forecast(session, tenant["tenant_id"], product, d)
    out = ForecastOut(available=fc is not None, reason=skip, product_id=str(product_id), as_of=d, target=TARGET)
    if fc:
