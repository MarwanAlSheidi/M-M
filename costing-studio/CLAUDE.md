...
- `make smoke` prints `SMOKE PASSED`: stage ok=24 flagged=1, inserted=24, golden_created=20,
  20 fixtures pass, /quote returns the tripwire landed_cost (26,720,146), no job running or failed, retrain skipped.
- `FIXES.md` lists every change.

## Sample data
`sample_data/tuna_25rows.csv` is generated by `scripts/gen_sample_csv.py` with the verified engine under
the seed config (insurance_rate 0.004); `make unit` fails if it is stale.
