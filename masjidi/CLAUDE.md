...
6. واجهة الويب في `app/` تغطي المسار كاملاً (`npm run dev` داخلها). المتبقّي:
   الخريطة، ورفع الصور، والعمل بلا إنترنت، ثم React Native إن لزم — وكلها
   تعيد استعمال `app/src/api.js` كما هو.
