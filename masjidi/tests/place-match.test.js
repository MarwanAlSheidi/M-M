...
test('مطابقة الاسم', async (t) => {
  ...
  /**
   * «الصحابي خالد بن الوليد» في بيانات الوزارة، و«مسجد خالد بن الوليد» في
   * الخريطة — مسجدٌ واحد تمنع مطابقتَه كلمةٌ واحدة. و63 من الـ430 المجهولة
   * موقعُها تبدأ بلقب. كشفه تقريرُ «كاد يطابق» على تجربةٍ اصطناعية.
   */
  await t.test('واللقبُ وصفٌ لا تعريف، فيُطرح', () => {
    assert.deepEqual(distinctiveWords('الصحابي خالد بن الوليد'), ['خالد', 'الوليد']);
    assert.equal(nameMatches('الصحابي خالد بن الوليد', 'مسجد خالد بن الوليد'), true);
    assert.equal(nameMatches('السيدة حفصة بنت عمر', 'مسجد حفصة بنت عمر'), true);
  });

  await t.test('ولا يُطرح إن لم يبقَ بعده إلا كلمة', () => {
    // «الشيخ سالم» بلا لقبه «سالم» — كلمةٌ شائعة تطابق ما ليس هو.
    // وتخفيفُ القاعدة لا يجوز أن يبلغ حدَّ إبطالها.
    assert.deepEqual(distinctiveWords('الشيخ سالم'), ['الشيخ', 'سالم']);
    assert.equal(nameMatches('الشيخ سالم', 'مسجد سالم'), false);
  });
...
test('اختيار الموقع', async (t) => {
  ...
  await t.test('ولا نتيجة: يُردّ بسببٍ مكتوب', () => {
    assert.match(chooseLocation(mosque, [], KNOWN).rejected, /لا مرشّح في الجوار/);
  });
  ...
  /**
   * شرطُ الاسم في اتجاهٍ واحد، فتزيدُه كلمةٌ ولا تضرّ. لكنّ الزيادة قد تكون
   * فارقاً: «الفردوس» و«الفردوس الأعلى» مسجدان لا مسجد. كشفته تجربةٌ على
   * البيانات الحقيقية: قُبل «الفردوس الأعلى» لمسجدٍ اسمُه «الفردوس».
   */
  await t.test('والمطابقُ تماماً يُقدَّم على من زاد عليه', () => {
    const { accepted } = chooseLocation({ name: 'مسجد الفردوس' }, [
      at('مسجد الفردوس الأعلى', 23.61, 58.51),
      at('مسجد الفردوس', 23.63, 58.53),
    ], KNOWN);
    assert.equal(accepted.placeName, 'مسجد الفردوس');
  });

  await t.test('وبلا مطابقٍ تام يبقى المحتوي، ويحكم بينهم اللبس', () => {
    const { accepted } = chooseLocation({ name: 'مسجد الفردوس' }, [
      at('مسجد الفردوس الأعلى', 23.61, 58.51),
    ], KNOWN);
    assert.equal(accepted.placeName, 'مسجد الفردوس الأعلى');

    const verdict = chooseLocation({ name: 'مسجد الفردوس' }, [
      at('مسجد الفردوس الأعلى', 23.61, 58.51),
      at('مسجد الفردوس الأدنى', 23.63, 58.53),
    ], KNOWN);
    assert.match(verdict.rejected, /متعدّدون/);
  });
   ...
   * المسجد ليس في الخريطة أصلاً؟ والفرق حاسم — الأوّل يُعالَج والثاني لا.
   */
  await t.test('وما رُدّ لاسمه يحمل معه أقربَ ما كاد يطابق', () => {
    const verdict = chooseLocation({ name: 'مسجد النور الساطع' }, [
      at('مسجد النور', 23.61, 58.51),
      at('مسجد الفردوس', 23.62, 58.52),
    ], KNOWN);

    assert.match(verdict.rejected, /لا اسم يطابق/);
    assert.deepEqual(verdict.nearMisses, [{ name: 'مسجد النور', shared: 1 }]);
  });
