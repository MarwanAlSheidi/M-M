...
test('الرحلة كاملة في متصفّح', options, async (t) => {
  ...
  await t.test('سجلّ المسجد يحكي ما جرى، ويراه المصلّي كما يراه الإمام', async () => {
    await onScreen(imam, 'سجلّ المسجد يحكي ما جرى', async () => {
      ...
      /*
       * وعن أيّ احتياجٍ يتكلّم كلُّ سطر.
       *
       * قِيس في مسجدٍ له ثلاثة احتياجات: تسعةُ أسطرٍ لا يقول واحدٌ منها موضوعَه
       * — «طلب صيانة جديد» ثلاثاً متطابقة. والشاشة هي التي سمّاها الكود نفسه
       * «أداةَ الشفافية».
       */
      assert.ok(entries.includes('تصليح إنارة الصحن'),
        `السجلّ لا يقول عن أيّ احتياجٍ يتكلّم:\n${entries}`);
  ...
  await t.test('والمشرف يُحذَّر أن الاعتماد يَنزع مسجداً من إمامٍ قائم', async () => {
    ...
    await onScreen(panel, 'المشرف يرى التحذير', async () => {
      ...
      // كم انتظر صاحبُ الطلب — `createdAt` كانت تعبر السلك ولا تُقرأ قطّ،
      // فيُراجَع طلبٌ عمره أربعون يوماً كما يُراجَع طلبُ اليوم.
      assert.match(text, /قُدّم اليوم/,
