...
test('شاشات الاكتشاف تعرض النوع', () => {
  ...
  /*
   * **ولا عنوانُ شاشةٍ كذلك — وهذا ما فات الحارس.**
   *
   * كان يفحص `<h3>` وحده، فمرّ أخضرَ على ثلاثة مواضع تُمرّر الاسم خاماً إلى
   * عنوان شاشة. وقِيس في متصفّح على مصلّى عيدين:
   *
   *     بطاقة «مساجدي»:      مصلى العيدين   ✓
   *     عنوان «طلبات الصيانة»: العيدين        ✗
   *     عنوان «سجلّ المسجد»:   سجلّ العيدين    ✗
   *
   * فالمسجد الواحد يُسمّى في الشاشة الواحدة باسمين: البطاقةُ تقول «مصلى
   * العيدين»، والعنوانُ الذي تفتحه بالضغط عليها يقول «العيدين».
   */
  const rawProps = screens.match(/mosqueName[=:]\s*\{?\s*(?:trail|mosque|selected)\.name\b/g) || [];
  assert.deepEqual(rawProps, [],
    `عنوانُ شاشةٍ يقرأ الاسم بلا نوعه: ${rawProps.join('، ')}`);

  // وضابطٌ سالب: الأداة تجد النمط لو وُجد فعلاً
  assert.match('mosqueName={trail.name}', /mosqueName[=:]\s*\{?\s*(?:trail|mosque|selected)\.name\b/,
    'النمط لا يلتقط الصيغة التي وُضع لها — الأداة عمياء');
});
