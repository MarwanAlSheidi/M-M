...
test('صندوق الوارد', async (t) => {
  ...
  /*
   * **قرارٌ نُقض بقياس** — وكان هذا الفحص يحرس نقيضَه.
   *
   * كُتب أوّلاً أنّ البثّ القريب **لا يُخزَّن**، والحجّة الباقة: سطرٌ لكل
   * متطوّعٍ عند كل طلب. والحجّة صحيحة، **والنتيجة أنّ أحداً لا يُبلَّغ**:
   * `Parse.Push` لا يصل اليوم — لا Installation مسجَّل — وهو مكتوبٌ في
   * `CLAUDE.md` مع نهيٍ صريح: «لا تبنِ مساراً يعتمد على وصول الدفع وحده».
   *
   * وقِيس على خادمٍ حقيقي: متطوّعٌ موقعُه نقطةُ المسجد نفسها، ثم نُشر احتياجٌ
   * تطوّعيّ فيه — **ووارده فارغ**. فيُبلَّغ المتبرّع الذي ضغط «هذا مسجدي»،
   * ولا يُبلَّغ من يستطيع العمل ويقف عنده.
   *
   * فصار يُحفظ **للعشرين الأقربَ لا للخمسمئة**: الحجّةُ محفوظةٌ والوصولُ واقع.
   */
  await t.test('والبثّ القريب يُحفظ — للعشرين الأقربَ لا للخمسمئة', async () => {
    for (let i = 0; i < 3; i += 1) {
    ...
    assert.ok(inbox().length > before,
      'نُشرت فرصةٌ تطوّعية ولم تبلغ متطوّعاً واحداً — والدفع لا يصل اليوم');
