...
| الاختبارات | ✅ 394 حالة على بديل Parse (`npm test`) + 246 اختبار تكامل على `parse-server` حقيقي فوق PostgreSQL ببيانات وزارة حقيقية (`npm run test:integration`) + 45 حالة في متصفّح حقيقي (`npm run test:e2e`) |
...
44. **اختبارُ حارسٍ يؤكّد رسالته لا حالَه.** «سقط» ليست نتيجة بل بدايةُ سؤال:
