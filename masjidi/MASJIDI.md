...
| الاختبارات | ✅ 270 حالة على بديل Parse (`npm test`) + 88 اختبار تكامل على `parse-server` حقيقي فوق PostgreSQL ببيانات وزارة حقيقية (`npm run test:integration`) + 22 حالة في متصفّح حقيقي (`npm run test:e2e`) |
