...
| الاختبارات | ✅ 409 حالة على بديل Parse (`npm test`) + 277 اختبار تكامل على `parse-server` حقيقي فوق PostgreSQL ببيانات وزارة حقيقية (`npm run test:integration`) + 69 حالة في متصفّح حقيقي (`npm run test:e2e`) |
...
45. **قائمةٌ مكتوبةٌ باليد في حارسٍ لا تنمو مع ما تحرسه.** وقع هذا الصنف
    **أربع مرّات**: البوّابة الحقلية كانت تنظر إلى خمسة أزواج وصندوق الوارد
    ...
    و`export_records` خارجها؛ وحارسُ أوّل نشرٍ يؤكّد نصّ «**خطوتان** لا
    يفعلهما هذا الأمر» واللوحة تطلب أربعاً — **فثبّت النقص بدل أن يكشفه**،
    وسقطت جدولةُ المهامّ وتفعيلُ رفع الملفات من المسار السريع كلِّه. والخطوات
    تُقرأ اليوم من `scripts/lib/manual.js`، ويقابلها بالمخرَج وبالوثيقة حارسان.
    **والحارس فيها لا يفشل — لا يُسأل**، وهذا أسوأ:
...
47. **حجّةٌ صحيحةٌ يوم كُتبت تُراجَع يوم يُنقض شرطُها.** التعليقُ المكتوب
    فوق قرارٍ يحفظ **الحكم** ولا يحفظ **شرطه**، فيبقى يُقرأ حجّةً بعد أن زال
    ما بُني عليه. قِيس: شاشةُ الإشعار كُتبت «قراءةً لا فعلاً» بحجّة «الأفعال
    لها شاشاتها — يهمّني في الفرص»، وكانت صحيحة؛ ثم صار نداءُ القرب يُحفظ في
    الوارد، **فصار الوارد قناةَ الدعوة**، و«الفرص» تُحسب من موقع المتطوّع
    **الآن** والدعوة أُرسلت بموقعه **حينها**. فمن دُعي باسمه إلى عملٍ قريب
    فتح الدعوة فلم يجد إلا «رجوع»، والشاشةُ التي أُحيل إليها آخرُ مكانٍ يظهر
    فيه العمل. **فكلُّ تغييرٍ يُسأل: أيُّ حجّةٍ قائمةٍ في المستودع كان هذا
    شرطَها؟** — والسؤال يقع ساعةَ التغيير، فبعدها لا يبقى إلا الحكم.

48. **الحماية التي لا تعمل أسوأ من غيابها.** قِيس: `Parse.Cloud.define(name,
    handler, { rateLimit })` صيغةٌ صحيحة يقبلها `parse-server` **وتُسجّل صفر
    حدود** — بلا خطأٍ ولا تحذير، لأن كود السحابة يُحمَّل قبل أن يُنشئ الإقلاع
    مصفوفةَ الحدود، و`Config.get` يبني كائناً جديداً في كل نداء فتُدفع إلى ما
    يُرمى. وكادت تُشحن: الشيفرة كُتبت وبُنيت الحزمة، **ولولا أن قِيست بنداءٍ
    حقيقيّ لقرأ المالكُ في المستودع أن الحدود مضبوطة وليست مضبوطة.**
    فكلُّ حمايةٍ تُضاف **تُقاس بانتهاكها** لا بوجود سطرها: يُطلب المنعُ فعلاً
    ويُنظر أيقع. **ولا يُقاس ذلك من `127.0.0.1`** — الحدُّ يتخطّى العودة
    الداخلية والمفتاحَ الرئيس صراحةً، فحارسٌ من هناك أخضرُ على حدٍّ ميّت.

49. **قاعدةٌ تُسنّ على الخادم تُقال في النموذج.** حدٌّ يُفرض ولا يُذكر يُكتشف
    **بالسقوط بعد دفع الثمن**: قِيس أن من ملأ التسجيل كلَّه ثم ضغط رُدّ بـ«كلمة
    المرور قصيرة» ولم يكن في النموذج حرفٌ يقول ذلك؛ والصورةُ أسوأ — ستّةُ
    ميغابايت تصعد من هاتفٍ على شبكةٍ محمولة **ثم** تُردّ. فيُقال الشرط قبل
    الضغطة، ويُردّ ما يُعرف أنه مردود **في الجهاز بلا نداء** (الباقة ٢٥ ألفاً).
    **ويُنشئ ذلك رقمين لشيءٍ واحد** — فيُقابَلان بحارس (`limits-parity`):
    الرقم يُقرأ من مصدره في `cloud/`، والنصُّ واحدٌ في الموضعين. ونموذجٌ يَعِد
    بما يُردّ أسوأ من نموذجٍ ساكت.

50. **ما يُعرَض على المشتري يُحرَس كما يُحرس ما يُنشر.** `app/demo/` نسخةٌ
    تُجرَّب باليد — الشاشاتُ نفسها فوق خادمٍ في المتصفّح — ودخلت المستودع بلا
    حارسٍ واحد: قِيس صفرُ ذكرٍ لها في `verify`. وهي **أوّلُ ما يلمسه المشتري**،
    وعطبٌ فيها يقع في وجهه لا في سجلّ. والصنفُ وقع ثلاث مرّات ساعةَ بُنيت
    (إحداثياتٌ من حقلٍ خطأ فمات القربُ صامتاً، واسمٌ خُمِّن بدل أن يُنسخ،
    ونصُّ إشعارٍ خام) — **وكشفها القياسُ باليد لا حارس**. فصار عليها حارسان:
    مقابلةُ ما يُنادى بما يُخدَم اشتقاقاً، وقيادةُ الحلقة كاملةً في متصفّحٍ
    بلا شبكة. **وكلُّ خادمٍ مُصغَّر يُعيد كتابة منطقٍ قائم يُقابَل بمصدره** —
    وإلا انحرف في صمت.
...
scripts/
  ...
  build_demo_page.js   `npm run demo` — نسخةُ العرض في صفحةٍ واحدة تحمل خادمَها
...
tests/
  ...
  limits-parity.test.js  حدُّ الشاشة هو حدُّ الخادم — رقمٌ واحدٌ ونصٌّ واحد
  demo-parity.test.js  نسخةُ العرض تخدم كلَّ ما يناديه التطبيق — ولا بقايا
  integration/
  ...
  e2e/                 متصفّح حقيقي فوق خادم حقيقي — `npm run test:e2e`
    ...
    invited-volunteer.test.js  من دُعي باسمه يستطيع أن يقول «نعم» من حيث دُعي
    assign-contractor.test.js  الإمام يبلغ الشركة المعتمدة من شاشته لا من دالّةٍ بلا زرّ
    stated-limits.test.js  الشرطُ يُقرأ قبل أن يُدفع ثمنُه — والردُّ بلا نداء
    demo.test.js       نسخةُ العرض: الحلقةُ كاملةً في صفحةٍ واحدة بلا شبكة
...
app/
  demo/                نسخةُ العرض: الشاشاتُ نفسها فوق خادمٍ في المتصفّح — للعرض لا للنشر
...
### 🔴 أقربُ الناس إلى العمل هو الوحيد الذي لا يُبلَّغ

`CLAUDE.md` يقول نصّاً — منذ جولاتٍ — **«لا تبنِ مساراً يعتمد على وصول الدفع
وحده»**، لأنّ `Parse.Push` لا يصل: لا Installation مسجَّل، والتطبيق لا يسجّله.

**وكان في المستودع مسارٌ واحد يخالفها**: `pushToNearbyVolunteers` تُنادى
بـ`{ store: false }`. وقِيس على خادمٍ حقيقي بالمسار الحقيقي لتسجيل الموقع
(`updateMyLocation` — فالعميل لا يملك كتابة `lastLat`):

```
موقع المتطوّع:  23.6 / 58.5   ← نقطةُ المسجد نفسها
نُشر احتياجٌ تطوّعيّ في ذلك المسجد
وارد المتطوّع:  (فارغ)
```

فمن هو **أقربُ الناس إلى العمل** لا يعلم به حتى يفتح التطبيق من تلقاء نفسه —
بينما يُبلَّغ المتبرّع الذي ضغط «هذا مسجدي»، ويُبلَّغ الإمام، ويُبلَّغ المنفّذ.
**والمرحلة الأولى كلُّها قائمةٌ على التطوّع العيني**، فهذه هي القناة التي عليها
المدار.

#### والحجّة التي أسقطت الحفظ كانت صحيحة — والنتيجة لم تكن

التعليق يقول: «خمسمائة سطر عند كل طلب جديد تُنهك باقة الطلبات». صحيح. **لكنّ
عملَ صيانةٍ في مسجدٍ يحتاج متطوّعاً واحداً لا خمسمئة.** فصار الحدُّ **العشرين
الأقربَ** — سخاءٌ لا شحّ، ويُكتبون في دفعةٍ واحدة (`saveAll` يجمع عشرين في
طلب). والخطة البديلة (بالمحافظة، حين لا إحداثيات) تُقيَّد بالعدد نفسه بعد أن
كانت خمسمئة.

فالحجّةُ محفوظة والوصولُ واقع.

#### وفحصان كانا يحرسان نقيضَ هذا

`tests/notifications.test.js` و`tests/integration/inbox.test.js` كلاهما يؤكّد
«البثّ الواسع لا يُخزَّن». **ولم يُحذفا بل قُلبا ومعهما سببُ القلب**: قرارٌ
مكتوبٌ في حارس يُنقض بقياسٍ لا بصمت. وهذا أوّلُ نقضٍ لقرارٍ محروس في هذه
الجلسة — فليُقرأ في مكانه من الملفّين.

وعلى `HEAD` (٨١٣٤ بايتاً لـ`push.js`): تسعُ حالاتٍ تمرّ، وتسقط اثنتان بالنصّ
المقيس — «أقربُ متطوّعٍ إلى العمل لم يُبلَّغ به: []».
...
### 🔴 فحصٌ يسمّي أشيعَ إخفاقٍ ثم لا يقدر على كشفه

الدورتان الماضيتان ضاعفتا ما يُكتب في `Notifications`. **فسُئل: من يكنسه؟**
والجواب مهمّتان دوريّتان (`pruneAuditLog`, `pruneNotifications`) هما الحاجز
الوحيد أمام نموّ القاعدة في باقةٍ سعتُها ٢٥٠ ميغابايت — **ولم تُجرَّبا قطّ إلا
على بديل Parse في الذاكرة** عبر `api.runJob`، وهو استدعاءٌ مباشر للدالّة لا
يقول شيئاً عن الخادم.

فقِيستا كما تُنادَيان من Back4app — `POST {serverURL}/jobs/:name` بالمفتاح
الرئيس — وكانت نتيجتان سالبتان: المنفَذ يُنادى (200)، و`_JobStatus` يُكتب.
**والثالثة كشفت العطب.**

#### خادمٌ عمرُه شهران يُقرأ في يومه الأول

```
خادمٌ عمرُه ٦٠ يوماً، لا صفَّ واحد في _JobStatus:
✓ المهام الدورية شُغّلت فعلاً — لم تُشغَّل بعد، طبيعيٌّ قبل أوّل موعد، وعطبٌ بعده
```

الفحص كان يقرأ `_JobStatus`، فإن خلا **مرّ أخضر إلى الأبد**؛ وحدُّ الأربعةَ
عشرَ يوماً لا يُطبَّق إلا على من شُغّل مرّةً ثم انقطع. أي أنه يكشف **انقطاع**
الجدولة ولا يكشف **ألّا تكون بدأت** — وهي الحالة التي سمّاها تعليقُه نفسه، في
السطر الذي فوقه، **«أشيعَ إخفاقٍ لهذه الخطوة»**. القاعدة مكتوبةٌ ولم تكن
منفَّذة.

فصار يُقاس **عمرُ الخادم نفسه** من أقدم مسجدٍ فيه — الاستيراد ثالثُ خطوات
النشر ويسبق أوّل مستخدم، ولا يُحذف المسجد بعدها. قبل أسبوعين يُقال «في يومه
كذا»، وبعدهما أحمر.

#### ومن كان يُخبر المُشغّل بالخطوة أصلاً؟

لا أحد. `npm run deploy` كان يقول «**خطوتان** لا يفعلهما هذا الأمر» ويعدّ لصقَ
كود السحابة والفهرسَ الفريد — و`docs/DEPLOY.md` يفصّل في بابيه الخامس والسادس
**خطوتين أخريين في اللوحة**: جدولةُ المهمّتين، وتفعيلُ رفع الملفات. وكلتاهما
ساقطةٌ من المسار السريع وحده — وهو المسار الذي يُقرأ مستعجلاً.

**ورابعُ وقوعٍ للقاعدة ٤٥:** `tests/integration/first-deploy.test.js` كان يؤكّد
نصّ «خطوتان» حرفياً — **فيثبّت النقص بدل أن يكشفه**. فصارت القائمة تُقرأ من
`scripts/lib/manual.js`، ويُقابَل بها مخرجُ الأمر ونصُّ الوثيقة معاً، ولكلّ
خطوةٍ **هل يكشفها `preflight`**: اثنتان نعم، ولصقُ الكود لا (بدونه لا يعمل
الفحص نفسه)، ورفعُ الملفات لا (يُعطي أخضرَ كاذباً بالمفتاح الرئيس).

#### وأداةُ القياس هي الحصيلة الباقية

حدودُ المدد في هذه المنصّة لم تكن قابلةً للقياس أصلاً: `createdAt` لا يُقدَّم
إلى الوراء عبر REST ولو بالمفتاح الرئيس — قِيس، فرُدّ بـ201 و`createdAt` هو
الآن. ويُقدَّم عبر `DatabaseController` في العملية نفسها. فصار حدُّ التسعين
يوماً مُجرَّباً على محوّلٍ حقيقي لا موصوفاً:

```
قبل: 5 (ثلاثةٌ عمرُها ١٢٠ يوماً)
POST /jobs/pruneNotifications → 200
_JobStatus: succeeded — حُذف 3 إشعاراً أقدم من 90 يوماً.
بعد: 2 — وكلُّ ما بقي «حديث»
```

**والحدّ الأخضر داخل الحارس مقصود**: مهمّةٌ تحذف كلَّ شيء تُبلّغ بنجاحٍ لا
يُميَّز عن الصواب، فيُعدّ الباقي كما يُعدّ الذاهب.

#### وحمرةٌ أولى لم تكن خبراً عن الوثيقة

أوّلُ صياغةٍ لحارس الوثيقة أخذت ما **قبل** أوّل عنوانٍ فرعيّ ظنّاً أنه المسار
السريع، وهو مقدّمةُ الوثيقة. فسقط على `Cloud Code` — أي على خطوةٍ **مذكورةٍ
فعلاً** — واحمرّ لسببٍ غير الذي وُضع له، وكاد يُقرأ بيّنةً على العطب.

---

### 🔴 دُعي باسمه إلى العمل، ولا يملك إلا أن ينصرف

الدورة ٥٠ جعلت نداء القرب يُحفظ في الوارد. **فسُئل: أيُّ حجّةٍ قائمةٍ كان هذا
شرطَها؟** — وكانت واحدة مكتوبةً فوق شاشة الإشعار: «قراءةً لا فعلاً، فالأفعال
لها شاشاتها: يهمّني في الفرص». صحيحةٌ يوم كُتبت، **ومنقوضةٌ يومها**: صار الوارد
قناةَ **الدعوة** لا الخبر وحده.

وقِيس في متصفّح حقيقي على متطوّعٍ كان عند المسجد ساعةَ النشر ثم فتح التطبيق من
بيته — وهو الحال الغالب لا النادر:

```
الوارد:   فرصة تطوّع: إصلاح مكبّر الصوت — جامع البلاغ
الدعوة:   إصلاح مكبّر الصوت · مفتوح للتطوّع · جامع البلاغ · كهرباء
الأزرار:  ["→ رجوع"]
الفرص:    «لا توجد فرص مفتوحة الآن.»
```

**أعلى لحظةِ نيّةٍ في المنتَج كلِّه**: أُخبر باسمه بعملٍ قريبٍ منه، ففتحه،
فلم يجد ما يضغطه. والشاشة التي أُحيل إليها هي آخرُ مكانٍ يظهر فيه العمل —
«الفرص» تُحسب من موقعه **الآن**، والدعوة أُرسلت بموقعه **حينها**.

فصار «يهمّني» يُعرض حيث وقعت الدعوة، **بشرطيه**: متطوّعٌ، واحتياجٌ لا يزال
مفتوحاً. والمتبرّع لا يُعرض عليه — والحارس الذي يؤكّد ذلك كان موجوداً قبل هذه
الدورة (`notified-request.test.js`) وبقي أخضر، فالتمييز بالدور قائمٌ لا مُخترَع.

#### وما كان يجب أن يبقى أخضر بقي

في الحارس الجديد حدّان لا يتعلّقان بالإصلاح: أنّ الدعوة تصل وتُفتح على عملها،
**وأنّ «الفرص» من بيته لا تعرضه**. والثاني هو **فرضُ القياس كلِّه**: لو صار
العمل يظهر هناك لبطل الاختبار — لا لأنه أُصلح بل لأن سببه سقط. وكلاهما أخضرُ
على `HEAD` وبعده، والحمرةُ كانت في الحالتين الأخريين وحدهما.

#### ونتيجةٌ سالبة تُقال ولا تُحوَّل عملاً

الضغطة تُقاس **بأثرها عند الإمام** لا برسالةٍ على الشاشة: سطرٌ في
`TaskInterests` على الطلب الذي دُعي إليه بعينه. والخادم كان يقبل هذا النداء من
هذا المتطوّع على هذا الطلب **قبل الإصلاح وبعده** — لا عطبَ فيه. **العطب كان
أنّ أحداً لا يعرضه عليه.**

---

### 🔴 مسارٌ مبنيٌّ من طرفيه، مقطوعٌ في وسطه

سؤالُ الدورة: **ما الذي بُني ولا مدخل له؟** والجواب مسارُ الشركات كلُّه.

قِيس في متصفّح حقيقي على إمامٍ عنده احتياجٌ مفتوح، وفي القاعدة شركةٌ معتمدة:
...
وقِيس على الخادم في اللحظة نفسها:

```
التكلفة: 0 · الحال: open_for_volunteers
تكليفُ الشركة على طلبٍ تطوّعيّ: **نجح** — الحال assigned · المكلَّف شركة
```

**فالطريق سالكٌ من طرفيه ومقطوعٌ في وسطه.** `expressInterest` للمتطوّعين
وحدهم — والشركة لا تُبدي اهتماماً **قصداً**: الإمام يختارها. وزرُّ التكليف
الوحيد في الشاشة يقرأ معرّفه من قائمة المهتمّين. **فلا دالّة في المستودع
كلِّه تُعطي الإمامَ معرّفَ شركة.**

وعلى هذه الخطوة الواحدة يتعلّق كلُّ ما بُني للشركات: التسجيل بالسجل التجاري،
وطابورُ الاعتماد عند المشرف، و`isVerifiedContractor`، و`assignedContractorId`،
وتبويب «مهامّي» عندها، و`startWork` و`markWorkDone`، واختبارُ التكامل
`contractor.test.js`. **وشركةٌ تُسجَّل وتُعتمد ثم لا يبلغها عملٌ أبداً تنصرف.**

#### ولم يكن هذا تأجيلاً إلى المرحلة الثانية

`PROJECT_SPEC.md` يقول «المرحلة 2 — الشركات المعتمدة». **لكنّ البناء لا يقول
ذلك**: التطبيق يفتح التسجيل بدور `contractor` اليوم، ويطلب السجل التجاري،
ويعرض للمشرف طابور اعتماد. فليست الشركات مؤجَّلة بل **نصفَ موصولة** — وهذا
أسوأ من التأجيل: المؤجَّل لا يَعِد أحداً.

وشركةٌ تتبرّع بعملها على احتياجٍ تكلفتُه صفر **لا مال فيها**، فلا تحتاج
تصريحاً — أي أنها مرحلةٌ أولى بامتياز. والمؤجَّل حقاً عروضُ الأسعار والتسعير،
وقد صُحّحت الوثيقة إلى ذلك.

#### وما لا يُعطى الإمام

الهاتف والسجل التجاري لا يعبران إلى هذه الشاشة. الدالّة تعمل بالمفتاح الرئيس
فتمرّ الحقول كلُّها ما لم تُنتقَ، والهاتف طريقُه `getRequestContacts` بعد
التكليف وفي مدّته وحدها (القاعدة ٣٠)، والسجلّ التجاري أساسُ الاعتماد عند
المشرف وهو محجوبٌ في `protectedFields`. ويُقاس ذلك في الحارسين معاً — بالحقول
على الخادم، وبالنصّ المعروض في المتصفّح.

#### وعطبٌ في الإصلاح نفسه قبل أن يُشحن

أوّلُ صياغةٍ للدالّة قرأت `noShowBy` من **الحساب** لتحسب الغياب. و`noShowBy`
على **الطلب** لا على الحساب؛ والمحسوب منه يُخزَّن في `abandonedJobs`. فكانت
تُعيد صفراً أبداً — أي **«لا غياب» على من تغيّب**، وهو أخطر من ألّا يُعرض
الحقل. ولم يكشفه اختبار: كشفه أنّ الحقل حين كُتب سُئل عن مصدره.

---

### 🔴 حمايةٌ كُتبت وبُنيت — ولا تفعل شيئاً على الإطلاق

سُئلتُ عن الثغرات فقستُ، وكان أوّلَ ما وجدتُ: **لا حدَّ معدّلٍ واحد في
المستودع كلِّه**. والباقة ٢٥ ألف طلبٍ شهرياً، فحسابٌ واحد في حلقةٍ يُحرقها في
دقائق — وتسقط المنصّة بلا سرقةِ بايت.

والصيغة المغرية أن يُشحن الحدُّ مع الكود:
`Parse.Cloud.define(name, handler, { rateLimit })` — فلا يحتاج لوحةً ولا مالكاً
يتذكّر. **فكُتبت، وبُنيت الحزمة، ومرّ الفحص.** ثم قِيست بنداءٍ حقيقيّ:
...
**صفرٌ. بلا خطأٍ ولا تحذير.** والآلية مفهومة: `define` ينادي
`addRateLimit(route, appId, true)` → `Config.get` **يبني كائناً جديداً في كل
نداء**؛ وكود السحابة يُحمَّل **قبل** أن يُنشئ الإقلاع مصفوفة `rateLimits` في
المخزون (`ParseServer.app` يبدأ بـ`rateLimit = []` فلا تدور حلقتُه) — فتُدفع
الحدود إلى مصفوفةٍ على كائنٍ يُرمى.

#### والنقيض قِيس ليُعرف موضعُ العطب
...
**الإنفاذ سليم. التسجيلُ من التحميل ميّتٌ صامت.** ولولا هذا القياس لَشُحن ما
يُقرأ حمايةً وليس حماية — **وذلك أسوأ من غيابها**: من يعلم أنه مكشوف يحتاط،
ومن قرأ في مستودعه أن الحدود مضبوطة لا يحتاط.

#### ولا يُقاس هذا من العودة الداخلية

`middlewares.js` يتخطّى صراحةً كلَّ طلبٍ مصدرُه `127.0.0.1`، ويتخطّى المفتاحَ
الرئيس. فحارسٌ يقيس من هناك يمرّ **أخضر على حدٍّ ميّت** — ولذلك يُشغَّل الحارس
بعنوانٍ خارجيّ، ويتخطّى نفسه ويقول لماذا إن لم يجد واحداً.

#### فما شُحن

`cloud/lib/limits.js` أُزيل من كود السحابة كلِّه وصار `scripts/lib/rate-limit.js`:
مصدرُ القيمة التي تُلصق في `PARSE_SERVER_RATE_LIMIT`، يطبعها `npm run deploy`
جاهزةً — ومعها بابا `/login` و`/users`، وهما **ليسا دالّتَي سحابة** فلا سبيل
إلى حدّهما إلا من هناك، وعليهما تخمينُ كلمات المرور وإغراقُ التسجيل.

وصارت **خامسةَ خطوات اللوحة** في `scripts/lib/manual.js` — فانتشرت وحدها إلى
مخرَج `deploy` وإلى الوثيقة وإلى حارسَيهما، وهي ثمرةُ ما بُني في الدورة ٥٢.

**وحارسٌ يثبّت المقيس**: أنّ التسجيل من التحميل يُسجّل صفراً (فلا يُعاد تأليف
الوهم)، وأنّ الإنفاذ يعمل حين يُسجَّل في حينه، وأنّ **كلَّ دالّة لها قرار** —
سياسةٌ أو سببٌ مكتوب، ولا قرارَ لدالّةٍ زالت.

#### والقاعدة ٤٨

**الحماية التي لا تعمل أسوأ من غيابها** — فكلُّ حمايةٍ تُضاف تُقاس
**بانتهاكها** لا بوجود سطرها.
...
### 🟠 قاعدةٌ سُنّت على الخادم، ولا يعرفها من يقف أمام النموذج

الدورة الماضية سنّت شرطين على الخادم — طولُ كلمة المرور، ونوعُ الصورة وحجمُها
— وكلاهما يردّ بالعربية. **فسُئل بالقاعدة ٤٧: أيُّ شيءٍ صار كاذباً بعدهما؟**
والجواب: النموذج.

```
نموذج التسجيل: ذكرٌ لطول كلمة المرور؟ **لا** · minLength؟ **لا شيء**
ثم بعد ملء الاسم والهاتف والصفة والمحافظة والمهارات والضغط:
«كلمة المرور قصيرة — 8 أحرف على الأقل.»
```

**فالقاعدة لا تُعرف إلا بالسقوط بعد دفع الثمن.** والصورةُ أسوأ: ستّةُ ميغابايت
تصعد كاملةً من هاتفٍ على شبكةٍ محمولة في السلطنة — **ثم** تُردّ. فيدفع المنفّذ
مرّتين: بياناتِه وانتظارَه.

#### فصارت تُقال قبل الضغطة

شرطُ كلمة المرور تحت الحقل في التسجيل وحده — **لا في الدخول**: حسابٌ أُنشئ قبل
سنّ القاعدة قد تكون كلمتُه أقصر، وتذكيرُه بشرطٍ لا يخصّه إقلاقٌ. وحدُّ الصور
تحت زرّ الاختيار، **والكبيرةُ لا تُضاف أصلاً** بدل أن تُرفع ثم تُردّ.

**والردُّ يقع في الجهاز بلا نداء** — ويُقاس ذلك بعدّ النداءات على الشبكة لا
بالنظر إلى الشاشة: صفرُ نداءاتٍ إلى `/users` أو `/login` أو `/functions`.
والباقة ٢٥ ألف طلبٍ شهرياً.

#### والخطرُ الذي أنشأه الإصلاح

رقمان لشيءٍ واحد: واحدٌ في `cloud/` **يفرض**، وواحدٌ في `app/` **يَعِد**. فإن
شُدّد الأوّل وبقي الثاني أوسعَ، صار النموذج يَعِد بما يُردّ عند أوّل ضغطة —
**وذلك أسوأ من نموذجٍ ساكت**. فحارسٌ يقرأ الرقم من مصدره في `cloud/` نصّاً
ويقابله بما تصدّره الواجهة، ويشترط أن يكون **النصُّ واحداً** في الموضعين: رسالتان
لعطبٍ واحد تُقرآن سببين.

#### وحمرةٌ لم تكن خبراً — للمرّة الثانية

أوّلُ صياغةٍ للحارس أكّدت وجود `LIMITS` عند تحميل الملفّ، فسقط الملفّ كلُّه على
`HEAD` قبل أن تبدأ حالةٌ واحدة: **صفرُ حالاتٍ ورمزُ سقوط**، لا يُميَّز من عطبٍ
في البيئة. وهو الدرس نفسه الذي وقع في الدورة الماضية حرفاً بحرف — فكُتب في
الملفّ لا في الذاكرة.

---

### 🟠 ما يُعرَض على المشتري دخل بلا حارس

الدورة الماضية أضافت `app/demo/` — نسخةٌ تُجرَّب باليد، الشاشاتُ نفسها فوق
خادمٍ يعمل داخل المتصفّح. فسُئل بالقاعدة ٤٧: **ما الذي صار هشّاً بإضافتها؟**

```
يناديها العميل: 31 · يخدمها العرض: 31 · ناقصة: لا شيء
أيلمس `verify` العرضَ؟ **صفرُ ذكرٍ في scripts/verify.js**
```

القائمتان متطابقتان اليوم — ولا شيء يُبقيهما كذلك. و`app/demo/server.js`
**يُعيد كتابة ما في `cloud/` بلغةِ المتصفّح**، فدالّةٌ تُضاف غداً في `api.js`
لا يعرفها العرض، ويُقال للمشتري في وجهه «دالّةٌ غير متاحة في العرض». وهي
**أوّلُ ما يلمسه**، وعطبٌ فيها يقع أمامه لا في سجلّ.

**وليس هذا احتياطاً من المستقبل**: الصنفُ وقع ثلاث مرّات ساعةَ بُنيت، وكشفها
القياسُ باليد لا حارس — إحداثياتٌ قُرئت من `lat` وهي في `location.latitude`
فمات القربُ صامتاً، واسمٌ خُمِّن بدل أن يُنسخ فأعطى «مصلى نساء مصلى النساء»،
ونصُّ إشعارٍ كتب الاسم خاماً.

#### فحارسان لا واحد

`tests/demo-parity.test.js` يقابل **ما يُنادى بما يُخدَم** اشتقاقاً من
المصدرين، ويفحص أنّ العيّنة تحمل إحداثيات داخل السلطنة، وأنّ قاعدة الاسم
منسوخةٌ لا مُخمَّنة. و`tests/e2e/demo.test.js` **يبني الصفحة من مصدرها ويقودها
كما يقودها المشتري**: متطوّعٌ يرى الفرصة ويسجّل اهتمامه، ثم يُبدَّل الحساب
فيراه القائمُ على المسجد ويكلّفه، ثم يعود فيجد التكليف في «مهامّي» — **وكلُّ
نداءٍ خارجيّ يُقطع**، فما يعمل هنا يعمل داخل إطارٍ معزول.

#### والضمُّ صار أمراً يُشغَّل

كان تجميعُ الصفحة في ملفٍّ واحد يجري **بيدي خارج المستودع**، فلا يُعاد ولا
يُفحص. وصار `npm run demo`، ويقيس الحارسُ ناتجَه هذا لا غيره. وكشف ذلك عطباً
في أوّل صياغةٍ للحارس: قِيست الحزمةُ **متفرّقةَ الملفات** فلم تُقلع أصلاً —
نصوصُ الوحدات على `file://` تُردّ بأصلٍ معتِم. فالمقيسُ يجب أن يكون **الشكل
المنشور** لا شكلاً آخر.

#### وحارسٌ لا يقدر على عطبه

أوّلُ صياغةٍ لشرط الاسم كانت `/(مصلى|جامع|مسجد)\s+\1/` — أي كلمةً تليها
نفسُها. فأُعيد العطب المقيس عمداً («مصلى نساء مصلى النساء») **ومرّ الحارس
أخضر**: الكلمتان بينهما كلمة. فصار الشرط أن يتكرّر لفظُ النوع في السطر.

**والقاعدة ٤٦ تُطبَّق بأثرٍ رجعيّ هنا**: أُعيدت الأعطابُ الثلاثة كلُّها واحداً
واحداً، وسقط الحارس في كلٍّ منها برسالته هو — لا بحمرةٍ مجهولة السبب.
...
- **المتطوّع لا يبلغ «كلُّ الفرص» ما دام موقعه يعمل.** `Opportunities` تشتقّ
  `nearby` من وجود الموقع وحده، فـ`openOpportunities` — وهي فرصُ السلطنة كلَّها
  — لا تُنادى إلا حين **يتعذّر** الموقع. فمن يعمل جهازُه محبوسٌ في خمسين
  كيلومتراً بلا زرٍّ يخرجه، ومن تعطّل جهازُه يرى أكثر منه. قِيس في الدورة ٥٣
  عرَضاً، ولم يُعالَج: مبدّلٌ صريح بين «قريبة منّي» و«كلّها» قرارُ تصميمٍ لا
  إصلاحُ عطب، والوارد صار يحمل الدعوة فسدّ أشدّ ما كان يترتّب عليه.
...
| `listApprovedContractors` | imam | الشركات المعتمدة ليختار منها منفّذاً — الطريق الوحيد إلى معرّف شركة |
...
# ناتجُ بناء نسخة العرض — يُولَّد من `app/demo/` ولا يُحفظ
app/demo-dist/
```
...
```json
{
  ...
  "scripts": {
    ...
    "demo": "node scripts/build_demo_page.js"
  },
  "dependencies": {
    "dotenv": "^16.4.5",
    "parse": "^5.3.0"
...
**والشركة تُكلَّف في المرحلة الأولى أيضاً** — على احتياجٍ تكلفتُه صفر، أي
شركةٍ تتبرّع بعملها. ولا مال يتحرّك فيه، فلا يلزمه تصريح. وكان هذا مبنيّاً على
الخادم ومقطوعاً في الشاشة: `assignWorker` يقبل الشركة، ولا دالّة تُعطي الإمامَ
معرّفها — فأُضيفت `listApprovedContractors`.

**المرحلة 2 — الشركات المعتمدة (بمقابل):** عروض الأسعار والتسعير، وما يتبعهما
من مقارنةٍ واختيار. أمّا التسجيل والاعتماد والتكليف والتنفيذ فقائمةٌ اليوم.
