...
| الاختبارات | ✅ 334 حالة على بديل Parse (`npm test`) + 177 اختبار تكامل على `parse-server` حقيقي فوق PostgreSQL ببيانات وزارة حقيقية (`npm run test:integration`) + 29 حالة في متصفّح حقيقي (`npm run test:e2e`) |
...
33. **من يقرّر يرى المدّة.** كلُّ طابورٍ ينتظر فيه إنسانٌ قرارَ إنسان يُعرض
    فيه «كم انتظر» — بـ`sinceLabel` لا بحسابٍ جديد. والحدُّ يُؤخذ من الوعد
    المقطوع في الخادم لا يُخترع. ويحرسه `tests/admin-fields.test.js`: ما
    تُرسله دوالُّ اللوحة يُقرأ فيها، أو يُكتب سببُ تركه في `ALLOWED_UNREAD`.
...
tests/
  ...
  admin-fields.test.js لوحة المشرف تقرأ ما يصلها — ولا حقلَ يعبر السلك بلا عين
