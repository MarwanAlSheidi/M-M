...
| الاختبارات | ✅ 259 حالة على بديل Parse (`npm test`) + 88 اختبار تكامل على `parse-server` حقيقي فوق PostgreSQL ببيانات وزارة حقيقية (`npm run test:integration`) + 21 حالة في متصفّح حقيقي (`npm run test:e2e`) |
...
scripts/
  ...
  resolve_locations.js استخراج مواقع المساجد المجهولة من خرائط جوجل — يكتب تراكباً لا قاعدة
  lib/place-match.js   قبول موقعٍ من جوجل أو ردُّه — نتيجتُه مرشَّحٌ لا حقيقة
...
tests/
  ...
  place-match.test.js  قواعد قبول موقع جوجل، وترتيب المصادر عند التعارض
  integration/
...
data/
  ...
  resolved_locations.json  ما استُخرج من خرائط جوجل — اختياري، يولّده resolve_locations.js
app/
...
### 🟢 مواقع المساجد لا تنتظر أئمّتها

**التوجيه:** «وكيل المسجد أو الإمام لن يكون عائقاً — سيسجَّل يدوياً بعدين.
مواقع المساجد موجودة في جوجل ماب بالاسم.»
...
فلا يُقبل المرشّح إلا باجتماع أربعة، وما رُدّ يبقى مجهولاً وسببُ ردّه مكتوب:

| الشرط | ما يمنعه |
|---|---|
| بيتُ صلاة (تصنيف جوجل، أو كلمةٌ في اسمه) | أن يصير موقع صيدليةٍ موقعَ مسجد |
...
**والشرط الأوّل لم أكتبه ابتداءً — كشفه اختبارٌ سقط.** كنت أختبر أن المطابق
...
# خرائط جوجل — اختياري.
# الخريطة في الواجهة تعمل بدونه (VITE_GOOGLE_MAPS_API_KEY في app/)، وهذا
# المفتاح لسكربت استخراج مواقع المساجد المجهولة من Places API.
# يحتاج تفعيل «Places API (New)»، والبحث النصّي مدفوع — انظر docs/DEPLOY.md
GOOGLE_MAPS_API_KEY=
```
