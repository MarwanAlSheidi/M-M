...
| الاختبارات | ✅ 377 حالة على بديل Parse (`npm test`) + 229 اختبار تكامل على `parse-server` حقيقي فوق PostgreSQL ببيانات وزارة حقيقية (`npm run test:integration`) + 44 حالة في متصفّح حقيقي (`npm run test:e2e`) |
...
سجلُّ الحدث لا اشتقاقٌ منه** — والفرق مقصود: القاعدة ٢٢ تمنع العدّادات التي
    ...
    },
    {
      ...
      "fields": {
        ...
        "contestedCount": {
          "type": "Number",
          "defaultValue": 0,
          "_comment": "كم أُبعد عن هذا المسجد لأن طلب هذا صاحبه كان معلّقاً — قرينةُ المشرف أن الملكية منازَعة"
        }
      },
      "classLevelPermissions": {
        "find": {},
