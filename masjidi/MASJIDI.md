...
| الاختبارات | ✅ 312 حالة على بديل Parse (`npm test`) + 146 اختبار تكامل على `parse-server` حقيقي فوق PostgreSQL ببيانات وزارة حقيقية (`npm run test:integration`) + 29 حالة في متصفّح حقيقي (`npm run test:e2e`) |
...
21. **ما يُشتقّ لا ينحرف.** العدّادات التي يُبنى عليها قرارٌ تُحسب باستعلامٍ
...
22. **حدُّ الطول على `beforeSave` لا في الدوال.** التسجيل يكتب على `_User`
