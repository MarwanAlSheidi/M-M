...
| الاختبارات | ✅ 403 حالة على بديل Parse (`npm test`) + 277 اختبار تكامل على `parse-server` حقيقي فوق PostgreSQL ببيانات وزارة حقيقية (`npm run test:integration`) + 61 حالة في متصفّح حقيقي (`npm run test:e2e`) |
