...
| الاختبارات | ✅ 343 حالة على بديل Parse (`npm test`) + 179 اختبار تكامل على `parse-server` حقيقي فوق PostgreSQL ببيانات وزارة حقيقية (`npm run test:integration`) + 31 حالة في متصفّح حقيقي (`npm run test:e2e`) |
...
tests/
  ...
  seed-guard.test.js   الاستيراد الكامل يُطلب باسمه — الباب الوحيد الذي لا رجعة منه
