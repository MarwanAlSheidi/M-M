...
## 13. الإثراء — عقد ملف الاستيراد

الشبكة محجوبة في بيئة التنفيذ، فمسار الاستيراد هو المسار العملي: تُجمَع صفحات
المنتجات خارجاً، ثم تُدمَج عبر نفس مسار التحقّق والإسناد الذي يمرّ به الجلب
المباشر — نفس الحالة، ونفس السبب، ونفس `provenance_url`.

```bash
cd eval
python scripts/enrich_catalog.py --write-import-template   # قالب بلا صفوف
python scripts/enrich_catalog.py --import <ملفّك>.csv
```
...
|---|---|---|
| `product_id` | نعم | يطابق `raw_catalog.csv` حرفاً بحرف |
| `product_url` | نعم | صفحة منتج، ويُقبل كذلك `source_url` أو `provenance_url` |
| `product_title_page` | لا | عنوان الصفحة كما هو |
| `description_page` | نعم فعلياً | بدونه الصفّ `FAILED` بسبب `IMPORT_NO_DESCRIPTION` |
| `category_page` | لا | نوع المنتج كما تذكره الصفحة |
| `fabric_page` | لا | |
| `color_page` | لا | |
| `variant_page` | لا | |
| `materials_raw` | لا | نصّ الخامات كما ورد |
| `features_raw` | لا | نصّ الخصائص كما ورد |
| `price_page` | لا | رقم فقط، بلا رمز عملة |
| `currency_page` | لا | `AED` مثلاً |
| `store_page` | لا | |
| `retrieved_at_source` | لا | وقت الجمع الفعلي في المصدر |

**الخلية الفارغة تعني «الصفحة لم تذكر هذا»**، وهي معلومة تُسجَّل ولا تُملأ
باستنتاج. لا تُشتقّ قيمة من العنوان: العنوان ليس وصفاً.

### قواعد إزالة التكرار

1. **هوية المنتج.** صفّان بنفس `product_id`: إن تطابق محتواهما حرفاً بحرف
   انطويا في واحد؛ وإن اختلفا **يُرفَض الملفّ كلّه** (`ValueError`). إبقاء
   الأول يعني الحكم بأنّ إحدى الروايتين صحيحة، وهذا ليس من صلاحية السكربت.
2. **العنوان القانوني.** `product_url_canonical` يسقط الـ query والـ fragment،
   ويصغّر البروتوكول والمضيف، ويحذف الشرطة الأخيرة — فـ `?variant=42` و
   `#reviews` و`/` تعني الصفحة ذاتها. أوّل سجلّ **بترتيب الكتالوج** (لا بترتيب
   ملفّ الاستيراد) يستحوذ على الصفحة، وأي سجلّ لاحق يشير إليها يصير `FAILED`
   بسبب `DUPLICATE_PRODUCT_URL` ولا يرث نصّها. صفحة واحدة لا تصلح دليلاً
   لمنتجين: النتيجتان ستتطابقان بحكم البناء لا بحكم القياس.

`provenance_url` يحتفظ بالعنوان كما ورد؛ الصيغة القانونية مفتاح مقارنة مشتقّ،
وليست بديلاً عن الأصل.

### حالة الإثراء المقيسة — لا شيء أُثري
...
| `NO_PRODUCT_URL` | 76 |
| `NETWORK_BLOCKED` | 1 |
| تغطية الوصف قبل/بعد | 1.3% / 1.3% |

**78 سجلاً تشير إلى 5 عناوين متمايزة فقط، اثنان منها صفحتا منتج.** أي أنّ 76
سجلاً تتقاسم ثلاث صفحات تجميعة، والسجلّ الوحيد الذي يملك عنوان منتج صالحاً
حُجبت شبكته. هذا سقف بنيوي في البيانات نفسها: لا يرفعه نموذجٌ أفضل ولا مُحاولة
جلب أخرى — يرفعه جمعٌ جديد يحمل عناوين `/products/...` حقيقية.
