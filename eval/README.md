...
| الرمز | المعنى | أمثلة |
|---|---|---|
| `0` | نجاح | تشغيلة كاملة · `--dry-run` · `--resume` · `--all-models` |
| `2` | فشل نزاهة يوقف التنفيذ | اختلاف بصمة · تباين المعرّفات · تكرار · تسرّب · انزياح عند الاستئناف |
| `3` | اختيار نموذج أو إعداد غير صالح | نموذج مجهول · نموذج **معطّل** · بيانات اعتماد ناقصة · ملف إعداد مفقود أو تالف |
| `4` | لا نموذج مؤهّل مع `--all-models` | كل النماذج معطّلة أو بلا مفاتيح |
| `5` | خطأ مصنّف | حزمة المزوّد غير مثبّتة |

النموذج المعطّل يخرج بـ `3` لا `5`: تعطيله **اختيار غير صالح** لا عطب في
المصنّف، والفحص يقع قبل بناء أي مزوّد فلا يُستدعى أبداً.
...
تشغيلتان متتاليتان للنموذج الوهمي تُنتجان **قيَماً عددية متطابقة تماماً**:
`headline` · `levels` · `selective` · `evidence` · `critical` ·
`error_analysis` · `confidence_intervals` · `per_field_operations` · التكلفة ·
الرموز · وترتيب السجلات.

**غير حتمي بقصد** (وليس عيباً):

| الحقل | السبب |
|---|---|
| `run_id` · `run_dir` | يحملان طابعاً زمنياً |
| `timestamp` · `generated_at` | وقت التشغيل |
| `*_latency_ms` | ساعة حائط: تقيس الجهاز لا النموذج |
